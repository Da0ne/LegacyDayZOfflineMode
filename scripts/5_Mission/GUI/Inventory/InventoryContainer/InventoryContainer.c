bool CanShowItemInInventory(PlayerBase player, Object obj)
{
	...
	// obj is NOT InventoryItem, Building, PlayerBase, ZombieBase, AnimalBase, TankOrCar
	if ( !obj.IsInherited(InventoryItem) && !obj.IsInherited(Building) && !obj.IsInherited(PlayerBase) && !obj.IsInherited(ZombieBase) && !obj.IsInherited(AnimalBase) && !obj.IsInherited(TankOrCar)  )
	{
		return false;
	}
	
	// obj is PlantBase, DiggedSoil
	if ( obj.IsKindOf("PlantBase") || obj.IsKindOf("DiggedSoil") || obj.IsKindOf("GardenPlot") ) // GardenPlot will be visible in vicinity in 063
	{
		return false;
	}
	...
	{
		PlayerBase obj_player = obj;

		if ( obj_player.GetLifeState() == LifeStateAlive && !obj_player.IsCaptive() )
		{
			return false;
		}
	}
	...
	if ( !obj.IsInherited(TentBase) && GetGame().IsInventoryTreeLocked(obj, player) )
	{
		return false;
	}
	
	if ( !GetGame().IsObjectAccesible(obj, player) )
	{
		return false;
	}
...
class InventoryContainer: ScriptedWidgetEventHandler
{
	...
	void OnItemDoubleClick(InventoryViewBase view, InventoryItem item)
	{
		...
		{
			if (player.CanTakeEntityAsOwned(item) && item.CanRemoveEntity())
			{
				player.TakeEntityAsOwned(item);
			}
		}
