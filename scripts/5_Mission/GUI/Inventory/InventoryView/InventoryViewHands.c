class InventoryViewHands extends InventoryView
{
	...
	void CreateLayout()
	{
		...
		else
		{
			...
			InitIconGrid();
			InitCargoGrid(m_entity.Ptr().GetCargo(), TYPE_PANEL_HANDS);
			...
			else if ( m_entity.Ptr().GetSlotsCount() > 0 )
			{
				m_IsExpandable = true;
			}
	...
	void InitIconGrid()
	{
		...
		if (m_icon_grid)
		{
			...
			// get sizes
			m_image_frame.GetScreenSize( image_frame_width, image_frame_height );
			GetGame().GetInventoryItemSize( m_entity.Ptr(), item_w, item_h );
			...
			//LogInfo(ToString( "InitIconGridHands:") + m_entity.Ptr().GetName() + " item_w=" + itoa(item_w) + ",item_h=" + itoa( item_h) + ",aw=" + ftoa(aw) + ",ah=" + ftoa(ah) + ",ww=" + itoa(ww) + ",hh=" + itoa(hh) + ",cargo="+ToString(m_entity.Ptr().GetCargo() )+ ",attachments="+itoa(  m_entity.Ptr().AttachmentsCount() ) + ",attachments="+itoa(  m_entity.Ptr().GetSlotsCount() ) , LogTemplates.TEMPLATE_JANOSIK );
			...
			m_icon_grid.GenerateBackgroundTiles(1);
			m_icon_grid.SetQuantityPanelSize( manager.GetQuantityStackPanelSize() );
			m_icon_grid.AddItem(m_entity.Ptr(), Vector(0, 1, 1), rotation );
		}
	}
	...
	void AlignToBottom()
	{
		...
		if ( menu && menu.IsInherited( InventoryMenu ) )
		{
			InventoryMenu inv_menu = menu;
			Widget center_panel = inv_menu.GetCenterPanel();
			if ( center_panel )
			{
				float w, h, cw, ch;
				m_root.GetSize( w, h );
