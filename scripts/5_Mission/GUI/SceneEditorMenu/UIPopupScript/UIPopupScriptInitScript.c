class UIPopupScriptInitScript extends UIPopupScript
{
	...
	void UIPopupScriptInitScript(Widget wgt)
	{
		...
		m_MedxInitScript	= wgt.FindAnyWidget("pnl_ppp_is_init_script_value");
		
		m_BtnRun			= wgt.FindAnyWidget("btn_ppp_is_run");
		m_BtnSave			= wgt.FindAnyWidget("btn_ppp_is_save");
		m_BtnCancel			= wgt.FindAnyWidget("btn_ppp_is_cancel");
	}
	...
	void OnOpen(Param param)
	{
		...
		if ( param != NULL )
		{
			Param2<int, SceneObject> param_scene_object = param;
