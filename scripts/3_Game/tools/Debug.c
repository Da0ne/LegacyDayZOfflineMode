...
{	
	...
	*/
	...
	static Shape DrawSphere(vector pos, float size = 1, int color = 0x1fff7f7f)
	{
		Shape shape = Shape.CreateSphere(color, ShapeFlags.TRANSP|ShapeFlags.NOOUTLINE, pos, size);
		m_DebugShapes.Insert(shape);
		return shape;
	}
	...
	*/
	...
	static private void LogMessage(string level, string plugin, string entity, string author, string label, string message)
	{
		...
		// Is exe parameter -enNoLogs
		if ( GetGame().IsNoLogs() == true )
		{
			return;
		}
