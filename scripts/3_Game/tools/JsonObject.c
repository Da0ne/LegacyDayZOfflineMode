class JsonObject
{	
	autoptr map<string, string> 	m_Strings;
	autoptr map<string, int>		m_Ints;
	autoptr map<string, float>		m_Floats;
	autoptr map<string, bool>		m_Bools;
	autoptr map<string, Vector2>	m_Vectors2;
	...
	void JsonObject()
	{
		...
		m_Vectors2	= new map<string, Vector2>;
	}
	...
	void Clear()
	{
		for ( int i = 0; i < m_Vectors2.Count(); ++i )
		{
			if ( m_Vectors2.GetElement(i) )
			{
				delete m_Vectors2.GetElement(i);
			}
		}
