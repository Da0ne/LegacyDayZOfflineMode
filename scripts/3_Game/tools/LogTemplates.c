...
class LogTemplates
{
	static private	map<LogTemplateID, LogTemplate> m_LogTemplates;
	...
	static private void	RegisterLogTamplate(LogTemplateID template_id, string author, string plugin, string label)
	{
		...
		{
			m_LogTemplates	= new map<LogTemplateID, LogTemplate>;
		}
	...
	{
		for ( int i = 0; i < m_LogTemplates.Count(); ++i )
		{
			LogTemplate t = m_LogTemplates.GetElement(i);
