...
/**
 ...
 @endcode
 */
class CallQueue extends array<CallQueueContext>
{
...
{
	private autoptr Param3<int, int, bool> m_mouse_params; // x,y, is_holding_mb
...
class TimerQueue extends array<TimerBase>
{
	...
	void ~TimerQueue()
	{
		if (Count())
		{
			for (int i = Count() - 1; i >= 0; i--)
			{
				TimerBase timer = Get(i);
				timer.OnTimerQueueDestoryed();
			}
...
/**
 \brief Simple class for fading Widgets
 */
class TimerUpdate extends TimerBase
{
	protected Class m_Target;
	protected string m_Function;

	// -------------------------------------------------------------------------
	void TimerUpdate(Class obj, string fn_name, int category)
	{
		m_Target = obj;
		m_Function = fn_name;
		OnInit(category);
	...
	{
		GetGame().GameScript.CallFunction(m_Target, m_Function, NULL, timeslice);
	}
};
