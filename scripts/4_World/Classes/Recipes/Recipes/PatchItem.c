...
{	
	void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"EN5C_LeatherSewingKit");//you can insert multiple ingredients this way
		InsertIngredient(0,"EN5C_SewingKit");//you can insert multiple ingredients this way
	...
	bool CanDo(EntityAI ingredients[], PlayerBase player)//final check for recipe's validity
	{
		...
		return module_repairing.CanRepair(ingredients[0],ingredients[1]);
	}

	void Do(EntityAI ingredients[], Man player,array<Object> results)//gets called upon recipe's completion
	{
		PluginRepairing module_repairing = GetPlugin(PluginRepairing);
		module_repairing.Repair(player, ingredients[0],ingredients[1],REPAIR_TYPE_TAILOR);
	}
};