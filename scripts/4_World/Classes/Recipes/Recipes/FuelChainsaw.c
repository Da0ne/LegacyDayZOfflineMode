...
{	
	void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"EN5C_Chainsaw");//you can insert multiple ingredients this way
	...
	bool CanDo(EntityAI ingredients[], PlayerBase player)//final check for recipe's validity
	{
		...
		int liquid_type = container.GetLiquidType2();
	...
	void Do(EntityAI ingredients[], Man player,array<Object> results)//gets called upon recipe's completion
	{
		...
		container.AddQuantity2(-can_add_amount);
	}
};