...
{	
	...
	void Update()
	{
		if ( m_Player )
		{
			...
			if ( player_stance == DayZPlayerConstants.STANCEIDX_CROUCH || player_stance == DayZPlayerConstants.STANCEIDX_RAISEDCROUCH) hud_stance_id = 2;
			if ( player_stance == DayZPlayerConstants.STANCEIDX_PRONE  || player_stance == DayZPlayerConstants.STANCEIDX_RAISEDPRONE)  hud_stance_id = 3;
	...
	{	
		if ( m_Player ) m_Player.GetDisplayStatus().SetStatus(DELM_STANCE, stance);
	}
};