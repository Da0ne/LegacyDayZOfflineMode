class EN5C_FishingRod_Base : ItemBase
{
	...
	{	
		...
		else
		{
			if ( m_FishingTime <= 0)
			{ 
				ItemBase pcatch = m_Player.CreateInInventory("EN5C_Carp","cargo_bait");
				pcatch.SetQuantity2(800,false);
				End(WIN);
			}
			else
			{
				...
				if ( m_Pull <= LOOSE_PULL) End(LOSS_LOOSE);
				if ( m_Pull >= BREAK_PULL) End(LOSS_BREAK);
			}
		}
	}
