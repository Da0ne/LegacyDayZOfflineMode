class PluginLocalProfile extends PluginFileHandler
{
	...
	map<string, map<string, string>>		m_ConfigParamsInArray;
	map<string, array<map<string, string>>>	m_ConfigParamsArrayInArray;
	
	void PluginLocalProfile()
	{
		m_ConfigParams				= new map<string, string>;
		m_ConfigParamsArray			= new map<string, TStringArray>;
		m_ConfigParamsInArray		= new map<string, map<string, string>>;
		m_ConfigParamsArrayInArray	= new map<string, array<map<string, string>>>;
	}
	...
	void ~PluginLocalProfile()
	{
		...
		// Deltete m_ConfigParams
		delete m_ConfigParams;
		
		// Deltete m_ConfigParamsArray
		for ( i = 0; i < m_ConfigParamsArray.Count(); ++i)
		{
			delete m_ConfigParamsArray.GetElement(i);
		}
		delete m_ConfigParamsArray;
		
		// Deltete m_ConfigParamsArrayInArray
		for ( i = 0; i < m_ConfigParamsArrayInArray.Count(); ++i)
		{
			array<map<string, string>> arrays = m_ConfigParamsArrayInArray.GetElement(i);
		...
		delete m_ConfigParamsArrayInArray;
	}
			...
			if ( array_items.Count() > 0 )
			{
				...
				for ( int j = 0; j < array_items.Count(); ++j )
				{
					...
					if ( array_content_raw.Count() > 0 )
					{
						...
						{
							m_ConfigParamsArrayInArray.Set(param_name, new array<map<string, string>>);
						}
						
						array<map<string, string>> params_in_array = m_ConfigParamsArrayInArray.Get(param_name);					
	...
	bool SaveConfigToFile()
	{
		...
		for ( i = 0; i < m_ConfigParamsArrayInArray.Count(); ++i)
		{
			...
			array<map<string, string>>	variable_param	= m_ConfigParamsArrayInArray.GetElement(i);
	...
	array<map<string, string>> ParseTextArrayToParameters(string text)
	{
		...
		array<map<string, string>> ret = new array<map<string, string>>;
	...
	string LoadSubParameter(string param_name, int sub_array_index, string sub_param_name)
	{
		...
		array<map<string, string>>	sub_arrays;
	...
	{
		array<map<string, string>> sub_arrays = m_ConfigParamsArrayInArray.Get(param_name);
	...
	TStringArray GetAllSubParametersInArrayString(string param_name, string sub_param_name)
	{
		...
		if ( m_ConfigParamsArrayInArray.Contains(param_name) )
		{
			...
			array<map<string, string>> arrays_in_parameter = m_ConfigParamsArrayInArray.Get(param_name);
