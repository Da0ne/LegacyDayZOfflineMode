...
{	
	protected const string SCENE_EDITOR_SCENE				= "scene_editor_scene";
	...
	protected const string SUB_PARAM_ITEM_DAMAGE			= "damage";
	protected const string SUB_PARAM_ITEM_QUANTITY			= "quantity";
	protected const string LOGS_ENABLED						= "logs_enabled";
	
	protected autoptr map<string, CfgParam> 				m_DefaultValues;
	protected autoptr TStringArray 							m_PresetList;	
	...
	{
		return GetPlugin(PluginConfigDebugProfile);
	}	
	
	//========================================
	// GetCfgParamArray
	//========================================
	protected CfgParamArray GetNewCfgParamArray( array<CfgParam> params )
	{
	...
	// Array
	//========================================
	protected CfgParamArray SetArray( string key, array<CfgParam> params )
	{
		CfgParamArray param = GetParamByName( key, CFG_TYPE_ARRAY );
		param.SetParams( params );
		SaveConfigToFile();
		return param;
	}

	protected array<CfgParam> GetArray( string key )
	{
		if ( ParamExist(key) )
		{
			CfgParamArray param = GetParamByName( key, CFG_TYPE_ARRAY );
		...
		else
		{
			...
			{
				CfgParamArray default_param = m_DefaultValues.Get( key );
				return SetArray( key, default_param.GetValues() ).GetValues();
			}
			else
			{
				return SetArray( key, new array<CfgParam> ).GetValues();
			}
		}
	}
	...
	{
		CfgParamBool param = GetParamByName( key, CFG_TYPE_BOOL );
		param.SetValue( value );
		SaveConfigToFile();
		return param;
	}
	...
	protected bool GetBool( string key )
	{
		...
		else
		{
			if ( m_DefaultValues.Contains(key) )
			{
				CfgParamBool default_param = m_DefaultValues.Get( key );
	...
	{
		CfgParamFloat param = GetParamByName( key, CFG_TYPE_FLOAT );
		param.SetValue( value );
		SaveConfigToFile();
		return param;
	}
	...
	protected float GetFloat( string key )
	{
		...
		else
		{
			if ( m_DefaultValues.Contains(key) )
			{
				CfgParamFloat default_param = m_DefaultValues.Get( key );
	...
	{
		CfgParamInt param = GetParamByName( key, CFG_TYPE_INT );
		param.SetValue( value );
		SaveConfigToFile();
		return param;
	}
	...
	protected int GetInt( string key )
	{
		...
		else
		{
			if ( m_DefaultValues.Contains(key) )
			{
				CfgParamInt default_param = m_DefaultValues.Get( key );
	...
	{
		CfgParamString param = GetParamByName( key, CFG_TYPE_STRING );
		param.SetValue( value );
		SaveConfigToFile();
		return param;
	}
	...
	protected string GetString( string key )
	{
		...
		else
		{
			if ( m_DefaultValues.Contains(key) )
			{
				CfgParamString default_param = m_DefaultValues.Get( key );
	...
	{
		array<CfgParam> items = GetArray( setting_name );
		for ( int i = 0; i < items.Count(); i++ )
		{
			CfgParamArray items_array = items.Get(i);
			array<CfgParam> item_params = items_array.GetValues();
			 
			for ( int j = 0; j < item_params.Count(); j++ )
			{
				if ( item_params.Get(j).GetName() == sub_param_name )
				{
					CfgParamString param_string = item_params.Get( j );
	...
	{
		m_DefaultValues = new map<string, CfgParam>;
		m_DefaultValues.Insert(SCENE_EDITOR_SCENE, 		GetNewCfgParamString(PluginSceneManager.SCENE_DEFAULT_NAME) );
	...
	void ~PluginConfigDebugProfile()
	{
		for ( int i = 0; i < m_DefaultValues.Count(); i++ )
		{
			delete m_DefaultValues.GetElement( i );
		}
	}
	...
	{
		super.OnInit();
		array<CfgParam> params = GetArray( PRESET_LIST );
		
		m_PresetList = new TStringArray;
		for ( int i = 0; i < params.Count(); i++ )
		{
			CfgParamString param = params.Get( i );
	...
	{
		CfgParamString cfg_scene_name = GetParamByName( SCENE_EDITOR_SCENE, CFG_TYPE_STRING );
	...
	bool PresetAdd( string preset_name )
	{
		...
		SetArray( preset_name, new array<CfgParam> );
		SaveConfigToFile();
	...
	{		
		...
		array<CfgParam> params = GetArray( preset_name );
	...
	protected array<CfgParam> GetItemParams( string preset_name, int item_index )
	{
		if ( m_PresetList.Find( preset_name ) == -1 )
		{
			return NULL;
		...
		CfgParamArray params_array = params.Get( item_index );
		array<CfgParam> item_params = params_array.GetValues();
	...
	{
		array<CfgParam> item_params = GetItemParams( preset_name, item_index );
	...
	float GetItemDamage( string preset_name, int item_index )
	{
		CfgParam param = GetItemParam( preset_name, item_index, SUB_PARAM_ITEM_DAMAGE );
		...
		{
			CfgParamFloat param_float = param;
			return param_float.GetValue();
		}
	}	
	
	bool SetItemDamage( string preset_name, int item_index, float damage )
	{
		CfgParam param = GetItemParam( preset_name, item_index, SUB_PARAM_ITEM_DAMAGE );
		...
		else
		{
			...
			if ( item_params != NULL )
			{
				CfgParamFloat new_param = new CfgParamFloat( SUB_PARAM_ITEM_DAMAGE );
				new_param.SetValue( damage );
