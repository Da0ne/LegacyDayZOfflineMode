class PluginConfigDebugProfileFixed extends PluginConfigDebugProfile
{
	...
	vector GetPositionByName( string pos_name )
	{
		...
		array<CfgParam> params = GetArray( POSITIONS_LIST );
		
		for ( int i = 0; i < params.Count(); i++ )
		{
			CfgParamArray params_array = params.Get( i );
			array<CfgParam> elements = params_array.GetValues();
