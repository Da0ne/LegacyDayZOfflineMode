...
{	
	array<CfgParam> m_CfgParams;
	
	void PluginConfigHandler()
	{
		m_CfgParams = new array<CfgParam>;
	...
	string ParseParam(local CfgParam param)
	{
		...
		switch ( param.GetType() )
		{
			case CFG_TYPE_ARRAY:
			{
				local CfgParamArray param_array = param;
				local array<CfgParam> params = param_array.GetValues();
			...
			{
				local CfgParamString param_string = param;
			...
			case CFG_TYPE_INT:
			{
				local CfgParamInt param_int = param;
			...
			case CFG_TYPE_FLOAT:
			{
				local CfgParamFloat param_float = param;
			...
			case CFG_TYPE_BOOL:
			{
				local CfgParamBool param_bool = param;
	...
	{
		local string param_name = STRING_EMPTY;
		local string param_value = text.Trim();;
		local int text_type = GetTextType(param_value);
		...
		{				
			...
			local CfgParamArray param_array = new CfgParamArray(param_name);
			local array<CfgParam> array_params = ParseToValueArray(param_value);
		...
		{					
			...
			local CfgParamString param_string = new CfgParamString(param_name);
		...
		{					
			...
			local CfgParamInt param_int = new CfgParamInt(param_name);
		...
		else if ( text_type == CFG_TYPE_FLOAT )
		{
			...
			local CfgParamFloat param_float = new CfgParamFloat(param_name);
	...
	protected array<CfgParam> ParseToValueArray(local string param_value)
	{
		local array<CfgParam> param_parent = new array<CfgParam>;
		
		local TStringArray strings = new TStringArray;
		
		local array<int> a = new array<int>;
		local array<int> b = new array<int>;

		local int pos_a = 0;
		local int pos_b = 0;
		local int element = 0;
		local int value_start = 0;
		local int value_end = 0;
		...
		if ( strings.Count() == 0 )
		{
			...
			local string param_value_content = param_value.Substring(1, param_value.Length() - 2);
		...
		{
			local string str = strings.Get(i);
