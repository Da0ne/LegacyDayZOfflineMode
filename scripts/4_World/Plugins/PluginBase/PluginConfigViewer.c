class PluginConfigViewer extends PluginBase
{
	...
	string GetBaseClassPath( string config_class_path, string class_path, string config_class )
	{
		...
		local autoptr TStringArray full_path = new TStringArray;
	...
	TStringArray GetConfigRecursive( local string path, local string name, local string config_class_path, local string config_class, local array<string> overridden, local int depth = 0, local bool check_base_class_of_class = false )
	{
			...
			if ( overridden.Find(child_name_lower) == -1 )
			{
				...
				if ( type != CT_CLASS )
				{
					...
					else
					{
						...
						local string tabs_array = MakeTabs( depth + 2 );
						local int j;
				...
				{
					local array<string> overridden_sub = new array<string>;
					
					if ( GetGame().ConfigGetChildrenCount(c_child_path) > 0 )
					{
						local TStringArray config1 = GetConfigRecursive( c_child_path, child_name, config_class_path, config_class, overridden_sub, depth + 1 );
					...
					local string cc_child_path = c_child_path;
					local string c_config_class_path = config_class_path;
					local string c_child_name = child_name;
					...
					while ( true ) 
					{
						...
						if ( base_class != "" )
						{
							...
							local string c_base_child_path = GetBaseClassPathCombined( cc_child_path, base_class );
							local string base_class_path = GetBaseClassPath( c_config_class_path, c_base_child_path, config_class );
							
							if ( base_class_path != "" )
							{
								local TStringArray config2 = GetConfigRecursive( base_class_path, base_class, base_class_path, config_class, overridden_sub, depth + 1, true );
								result.InsertAll( config2 );
								delete config2;
							}
							else
							{
								local TStringArray config3 = GetConfigRecursive( base_class, base_class, base_class, config_class, overridden_sub, depth + 1, true );
		...
		{
			local TStringArray full_path = new TStringArray;
			GetGame().ConfigGetFullPath( child_path, full_path );

			if ( full_path.Count() > 1 )
			{
				local TStringArray config4 = GetConfigRecursive( config_class + " " + full_path.Get(1), full_path.Get(1), config_class + " " + full_path.Get(1), config_class, overridden, depth );
		...
		{
			local string class_base_class = "";
			GetGame().ConfigGetBaseName( child_path, class_base_class );
			
			if ( class_base_class != "" )
			{
				local string base_child_path = GetBaseClassPathCombined( child_path, class_base_class );
				local string cc_base_child_path = GetBaseClassPath( config_class_path, base_child_path, config_class );

				if ( cc_base_child_path != "" )
				{
					local TStringArray config5 = GetConfigRecursive( cc_base_child_path, class_base_class, cc_base_child_path, config_class, overridden, depth, true );
					result.InsertAll( config5 );
					delete config5;
				}
				else
				{
					local TStringArray config6 = GetConfigRecursive( class_base_class, class_base_class, class_base_class, config_class, overridden, depth, true );
	...
	TStringArray GetConfig( string class_path, string filter_char )
	{
		...
		if ( class_path != "" )
		{
			...
			if ( path_nodes.Count() >= 3 )
			{
				...
				autoptr array<string> overridden = new array<string>;
				...
				delete nested_start;
				delete nested_end;
				delete nested_inherited_start;
				delete nested_inherited_end;
